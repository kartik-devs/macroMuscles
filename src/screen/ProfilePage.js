    ...
    const loadUserData = async () => {
      try {
        ...
        if (id) {
          ...
          const measurements = await getBodyMeasurements(id);
          setBodyMeasurements(measurements);
          
          // Load weekly progress
          const progress = await getWeeklyProgress(id);
          setWeeklyProgress(progress);
          
          // Load recent activity
          const activity = await getRecentActivity(id, 5);
          setRecentActivity(activity);
          
          // Load weekly calendar
          const calendar = await getWeeklyCalendar(id);
