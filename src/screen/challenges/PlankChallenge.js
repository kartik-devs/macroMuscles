...
export default function PlankChallenge({ navigation }) {
  ...
  useEffect(() => {
    const loadUserAndHistory = async () => {
      try {
        ...
        if (id) {
          const progressHistory = await getChallengeProgress(id, 'Plank Challenge');
        ...
        console.error('Error loading plank history:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const stopTimer = async () => {
    if (isRunning) {
      ...
      const newSession = { time: timer, date: new Date() };
      ...
      if (userId) {
        try {
          await saveChallengeProgress({
            user_id: userId,
            challenge_name: 'Plank Challenge',
            duration: timer
          });
        } catch (error) {
          console.error('Error saving plank progress:', error);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Plank Challenge</Text>
        <View style={{ width: 32 }} />
      </View>
      <ScrollView showsVerticalScrollIndicator={false} style={styles.scrollView}>
        <View style={styles.challengeCard}>
          <Ionicons name="flame" size={48} color="#e91e63" style={{ marginBottom: 16 }} />
          <Text style={styles.challengeTitle}>Hold your plank as long as you can!</Text>
        ...
        <View style={styles.historySection}>
          ...
          {loading ? (
            ...
            <Text style={styles.noHistoryText}>No plank sessions yet.</Text>
          ) : (
            history.map((entry, idx) => (
              <View key={idx} style={styles.historyItem}>
                <Ionicons name="time-outline" size={20} color="#e91e63" style={{ marginRight: 10 }} />
